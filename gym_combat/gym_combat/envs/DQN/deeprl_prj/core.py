...
#        self.screens[self.current % self.memory_size] = state
    ...
    def clear(self):
        ...
        if False:
            ...
            s = self.get_state(idx)
            ns = self.get_new_state(idx)
            # s = np.transpose(self.state[index - self.history_length + 1:index + 1, :, :], (1, 2, 0))
            # ns = np.transpose(self.new_state[index - self.history_length + 1:index + 1, :, :], (1, 2, 0))
            axs[1, 0].matshow(s)
            axs[1, 1].matshow(ns)
